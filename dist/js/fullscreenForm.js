...
;( function( window ) {
	...
	FForm.prototype.start = function(message) {
		document.body.querySelector('.fs-intro').innerHTML = message;
	};

	FForm.prototype.end = function(message) {
		document.body.querySelector('.fs-end').innerHTML = message;
	};

	FForm.prototype.ask = function(message) {
		var li = document.createElement('li');
		li.innerHTML = '\
		<label class="fs-field-label fs-anim-upper" for="q1">'+message+'</label>\
		<input class="fs-anim-lower" type="text" required/>';
		this.fieldsList.appendChild(li);
		this.fields.push(li);
		this.fieldsCount++;
	};

	FForm.prototype.choose = function(message) {
		var self = this;
		var li = document.createElement('li');

		var html = '\
		<label class="fs-field-label fs-anim-upper">'+message+'</label>\
		<div class="fs-radio-group fs-radio-custom clearfix fs-anim-lower">';

		for(var i=1; i<arguments.length; i++) {
			var id = this.fieldsCount+'-'+i;
			html += '<span><input id="'+id+'" type="radio" value="'+arguments[i]+'"/><label for="'+id+'" class="radio-conversion">'+arguments[i]+'</label></span>';
		}
		html += '</div>';
		li.innerHTML = html;
		[].slice.call( li.querySelectorAll( 'input[type="radio"]' ) ).forEach( function( inp ) {
			inp.addEventListener( 'change', function(ev) { self._checkNext(); } );
		} ); 
		this.fieldsList.appendChild(li);
		this.fields.push(li);
		this.fieldsCount++;
	...
	FForm.prototype.say = function(title, message) {
		if(!title) title = '';
		if(!message) message = '';
		var li = document.createElement('li');
		li.innerHTML = '\
		<label class="fs-field-label fs-anim-upper fs-say">'+title+'</label>\
		<span class="fs-anim-lower fs-sub-message">'+message+'</span>';
		this.fieldsList.appendChild(li);
		this.fields.push(li);
		this.fieldsCount++;
	};
	...
	FForm.prototype._checkNext = function() {
		console.log('_checkNext');
		if(!this._validade() || this.isAnimating) return false;
		console.log('valid');
		var currentFld = this.fields[ this.current ];

		var textInput = currentFld.querySelector('input[type="text"]');
		var radioInput = currentFld.querySelector('input[type="radio"]');
		var optionInput = currentFld.querySelector('input:checked');
		if(textInput) {
			iterator.next(textInput.value);
		}
		else if(radioInput) {
			if(optionInput && optionInput.value) {
				iterator.next(optionInput.value);
			}
			else {
				this._showError('NOVAL');
				return false;
			}
		}
		else {
		...
		var self = this,
			onEndAnimationFn = function( ev ) {
				...
				if( self.isLastStep ) {
					...
					var input = nextField.querySelector('input[type="text"]');
					if(input) {
						input.focus();
					}
				}
	...
	FForm.prototype._hideCtrl = function( ctrl ) {
		if(ctrl) classie.remove( ctrl, 'fs-show' );
	}
